...
def get_driver(dir):
    ...
    if platform == "linux":
        ...
        options.add_argument(
            f"download.default_directory={os.path.abspath(data_path)}")
        ...
        prefs = {
            "download.default_directory": str(ROOT_DIR.joinpath('data', dir))}
...
def get_sign_in_page(url, dir):
    ...
    try:
        ...
        WebDriverWait(driver, 10).until(
            EC.title_contains("Market Management System"))
...
def login(url, username, password, dir):
    ...
    try:
        ...
        page.find_element(
            by=By.XPATH,
            value='//*[@id="mat-expansion-panel-header-0"]/span').click()
        time.sleep(r_time)

        page.find_element(
            by=By.ID,
            value="mat-input-0").send_keys(username)
        time.sleep(r_time)

        page.find_element(
            by=By.ID,
            value="mat-input-1").send_keys(password)
        time.sleep(r_time)

        page.find_element(
            by=By.XPATH,
            value='//*[@id="cdk-accordion-child-0"]/div/div/button').click()
        time.sleep(r_time)
...
def select_prod(page, prod):
    try:
        ...
        prod_selector = page.find_element(
            by=By.XPATH,
            value='//*[@id="navbar-top"]/div[2]/mms-company-role-selector')
        prod_selector.click()
        cdk_overlay_panel = page.find_element(
            by=By.XPATH,
            value='//*[@id="cdk-overlay-0"]')
        time.sleep(r_time)
        logger.debug(f"try to select {prod} element")
        for option in cdk_overlay_panel.find_elements(by=By.TAG_NAME,
                                                      value='mat-option'):
            if option.text == prod:
                option.click()
        time.sleep(r_time)
...
def switch_frame(page):
    try:
        ...
        page.switch_to.frame(
            page.find_element(by=By.CSS_SELECTOR,
                              value='body > mms-root >'
                                    ' mms-full-layout > '
...
def get_balance_groups(page):
    ...
    try:
        ...
        page.find_element(by=By.NAME,
                          value='balanceGroupFilter').click()
        balance_group_selector = page.find_element(by=By.NAME,
                                                   value='balanceGroupFilter')
...
def select_day(page, day):
    try:
        ...
        page.find_element(by=By.NAME,
                          value='dateTypeChooser.day').click()
        date_chooser_day = page.find_element(by=By.NAME,
                                             value='dateTypeChooser.day')
        logger.debug(f"try to set day selector")
        for option in date_chooser_day.find_elements(By.TAG_NAME,
                                                     value='option'):
            if option.text == day:
                option.click()
        time.sleep(r_time)
...
def select_month(page, month):
    try:
        logger.debug(f"try to find day selector")
        page.find_element(by=By.NAME,
                          value='dateTypeChooser.month').click()
        date_chooser_month = page.find_element(by=By.NAME,
                                               value='dateTypeChooser.month')
        logger.debug(f"try to set day selector")
        for option in date_chooser_month.find_elements(by=By.TAG_NAME,
                                                       value='option'):
...
def select_year(page, year):
    try:
        ...
        page.find_element(by=By.NAME,
                          value='dateTypeChooser.year').click()
        date_chooser_year = page.find_element(by=By.NAME,
                                              value='dateTypeChooser.year')
        logger.debug(f"try to set day selector")
        for option in date_chooser_year.find_elements(by=By.TAG_NAME,
                                                      value='option'):
            if option.text == year:
                option.click()
                time.sleep(r_time)
...
def click_filter_button(page):
    ...
    try:
        ...
        page.find_element(by=By.XPATH,
                          value='//*[@id="filterButton"]').click()
        time.sleep(r_time + 2)
        return page
    except Exception:
...
def click_choices_button(page):
    ...
    try:
        ...
        page.find_element(by=By.XPATH,
                          value='/html/body/div/div/form/div[8]'
                                '/table/tbody/tr[3]/td[5]/a[1]'
                          ).click()
        time.sleep(r_time)
        return page
    except Exception:
...
def click_expand_group_button(page):
    ...
    try:
        ...
        page.find_element(by=By.XPATH,
                          value='/html/body/div/div/form/div[10]/nobr/input[2]'
                          ).click()
        time.sleep(r_time)
        return page
    except Exception:
...
def click_download_button(page):
    try:
        ...
        page.find_element(by=By.XPATH,
                          value='/html/body/div/div/form/div[7]/div/input[1]'
                          ).click()
        time.sleep(r_time)
        return page
    except Exception:
...
def click_back(page):
    try:
        ...
        page.find_element(by=By.XPATH,
                          value='/html/body/div/div/form/div[4]/a'
                          ).click()
        time.sleep(r_time)
        return page
    except Exception:
